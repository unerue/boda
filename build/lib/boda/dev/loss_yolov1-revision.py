...
class Yolov1Loss(LossFunction):
    ...
    def forward(
        ...
        preds = inputs[coord_mask].view(-1, 30)
        pred_boxes = preds[:, :5*2].contiguous().view(-1, 5)
        pred_labels = preds[:, 5*2:]

        trues = targets[coord_mask].view(-1, 30)
        true_boxes = trues[:, :5*2].contiguous().view(-1, 5)
        true_labels = trues[:, 5*2:]
        ...
        noobj_conf_mask = torch.cuda.BoolTensor(pred_noobj.size()).fill_(False) # [n_noobj, N]
        for b in range(2):
            noobj_conf_mask[:, 4 + b*5] = True # noobj_conf_mask[:, 4] = 1; noobj_conf_mask[:, 9] = 1
        
        pred_noobj_conf = pred_noobj[noobj_conf_mask]       # [n_noobj, 2=len([conf1, conf2])]
        true_noobj_conf = true_noobj[noobj_conf_mask]   # [n_noobj, 2=len([conf1, conf2])]
        loss_noobj = F.mse_loss(pred_noobj_conf, true_noobj_conf, reduction='sum')

        # Compute loss for the cells with objects.
        coord_response_mask = torch.cuda.BoolTensor(pred_boxes.size()).fill_(False)    # [n_coord x B, 5]
        coord_not_response_mask = torch.cuda.BoolTensor(true_boxes.size()).fill_(True)# [n_coord x B, 5]
        bbox_target_iou = torch.zeros(true_boxes.size()).cuda()                    # [n_coord x B, 5], only the last 1=(conf,) is used

        # Choose the predicted bbox having the highest IoU for each target bbox.
        for i in range(0, true_boxes.size(0), 2):
            pred = pred_boxes[i:i+2] # predicted bboxes at i-th cell, [B, 5=len([x, y, w, h, conf])]
            pred_xyxy = torch.cuda.FloatTensor(pred.size()) # [B, 5=len([x1, y1, x2, y2, conf])]
            # Because (center_x,center_y)=pred[:, 2] and (w,h)=pred[:,2:4] are normalized for cell-size and image-size respectively,
            # rescale (center_x,center_y) for the image-size to compute IoU correctly.
            pred_xyxy[:,  :2] = pred[:, :2]/float(7) - 0.5 * pred[:, 2:4]
            pred_xyxy[:, 2:4] = pred[:, :2]/float(7) + 0.5 * pred[:, 2:4]

            target = true_boxes[i]  # target bbox at i-th cell. Because target boxes contained by each cell are identical in current implementation, enough to extract the first one.
            target = true_boxes[i].view(-1, 5)  # target bbox at i-th cell, [1, 5=len([x, y, w, h, conf])]
            target_xyxy = torch.cuda.FloatTensor(target.size())  # [1, 5=len([x1, y1, x2, y2, conf])]
            # Because (center_x,center_y)=target[:, 2] and (w,h)=target[:,2:4] are normalized for cell-size and image-size respectively,
            # rescale (center_x,center_y) for the image-size to compute IoU correctly.
            target_xyxy[:,  :2] = target[:, :2]/float(7) - 0.5 * target[:, 2:4]
            target_xyxy[:, 2:4] = target[:, :2]/float(7) + 0.5 * target[:, 2:4]
        ...
        bbox_pred_response = pred_boxes[coord_response_mask].view(-1, 5)      # [n_response, 5]
        bbox_target_response = true_boxes[coord_response_mask].view(-1, 5)  # [n_response, 5], only the first 4=(x, y, w, h) are used
