...
# from .utils import preproc, bboxes_iou, postprocess
# from ..pafpn import YOLOPAFPN
# from ..backbone_darknet import DWConv, BaseConv
# from ..backbone_darknet import cspdarknet
# from .loss_yolox import IOUloss
...
#                 if p[1] == 'backbone':
#                     if p[2] == 'stem':
#                         new_key = f"backbone.layers.0.{'.'.join(p[3:])}"
#                         state_dict[new_key] = state_dict.pop(key)
#                     elif p[2].startswith('dark'):
#                         new_key = f"backbone.layers.{int(p[2][-1])-1}.{'.'.join(p[3:])}"
#                         state_dict[new_key] = state_dict.pop(key)
#                 else:
#                     new_key = f"neck.{'.'.join(p[1:])}"
#                     state_dict[new_key] = state_dict.pop(key)
