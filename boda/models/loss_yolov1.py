...
class Yolov1Loss(LossFunction):
    ...
    def encode(self, targets, num_grids=7, num_boxes=2, num_classes=7):
        ...
        outputs = torch.zeros(batch_size, num_grids, num_grids, 5*nb+nc, device='cuda')
    ...
    def forward(
        ...
        # print(inputs['boxes'].size(), inputs['scores'].size(), inputs['labels'].size())
        bs = inputs['boxes'].size(0)
        gs = 7
        nb = 2
        nc = inputs['labels'].size(2)
        ...
        coord_response_mask = torch.zeros_like(true_scores, dtype=torch.bool, device=true_scores.device)
        coord_not_response_mask = torch.ones_like(true_scores, dtype=torch.bool, device=true_scores.device)
