...
def xyxy2xywh(boxes: torch.Tensor) -> torch.Tensor:
    ...
    boxes: (B, 1, 4) -> [[[x, y, w, h]]] -> [[[x, y, x, y]]]
    """
    xywh_boxes = torch.zeros_like(boxes)
    xywh_boxes[..., 2:] = torch.abs(boxes[..., :2] - boxes[..., 2:])  # x, y
    xywh_boxes[..., :2] = boxes[..., :2] + xywh_boxes[..., 2:]/2  # w, h

    return xywh_boxes


def rescale_boxes(boxes: torch.Tensor, current_dim: int, original_shape: Tuple[int]):
